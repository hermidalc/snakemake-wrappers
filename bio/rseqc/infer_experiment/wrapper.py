...
print(f"Getting strandedness from {infer_file}")
fwd_str = "1++,1--,2+-,2-+"
rev_str = "1+-,1-+,2++,2--"
...
fracs = []
with open(infer_file, "r") as f_in:
    for line in f_in:
...
if np.all(percents < unstranded_p_thres):
    stranded = 0
elif np.any(percents > stranded_p_thres) and np.any(percents < 100 - stranded_p_thres):
...
with open(strand_file, "w") as f_out:
    f_out.write("forward" if stranded == 1 else "reverse" if stranded == 2 else "no")
